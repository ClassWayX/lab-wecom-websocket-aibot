...
sequenceDiagram
    ...
    note over D, W: 连接建立阶段
    D->>W: 建立WebSocket连接 (albot_subscribe)
    W-->>D: 连接建立成功
    end

    rect rgb(255, 243, 224)
    note over D, U: 进入会话事件
    U->>W: 进入机器人会话
    W->>D: 事件回调 (albot_event_callback)
    D->>W: 回复欢迎语 (albot_respond_welcome_msg)
    W->>U: 展示欢迎消息
    end

    rect rgb(232, 245, 233)
    note over D, U: 消息回调与流式消息
    U->>W: @机器人发消息
    W->>D: 消息回调 (albot_msg_callback)
    D->>W: 回复流式消息 (albot_respond_msg, finish=false)
    W->>U: 展示流式消息
    loop 开发者主动推送更新
        D->>W: 更新流式内容 (albot_respond_msg, finish=false)
        W->>U: 更新流式消息
    end
    D->>W: 完成流式消息 (albot_respond_msg, finish=true)
    W->>U: 展示最终消息
    end

    rect rgb(237, 231, 246)
    note over D, U: 模板卡片交互
    U->>W: 点击模板卡片按钮
    W->>D: 事件回调 (albot_event_callback)
    D->>W: 更新卡片 (albot_respond_update_msg)
    W->>U: 展示更新后的卡片
    end

    rect rgb(251, 233, 231)
    note over D, U: 主动推送消息 (无回调触发)
    D->>W: 主动推送消息 (albot_send_msg)
    W->>U: 展示推送消息
    end
...
```mermaid
sequenceDiagram
    participant A as 开发者服务
    participant B as 企业微信

    A->>B: 1. 发起 WebSocket 连接<br/>wss://openws.work.weixin.qq.com
    B-->>A: 2. WebSocket 握手成功
    A->>B: 3. 发送订阅请求 (aibot_subscribe)<br/>携带 BotID 和 Secret
    Note right of B: 4. 校验凭证
    B-->>A: 5. 返回订阅结果
    Note over A,B: 连接建立完成，开始接收回调
```
...
| body.from.userid | string | 消息发送者的 userid。如果智能机器人创建者为企业超级管理员，则此字段为明文userid，否则此字段为企业主体下的加密userid（可通过 [自建应用与智能机器人的对接](#61328) 转为明文userid） |
...
| body.chatid          | string | 会话 ID，仅群聊类型时返回                                                                                                                                                         |
| body.chattype        | string | 会话类型， `single` 单聊 / `group` 群聊                                                                                                                                           |
| body.from.userid     | string | 事件触发者的 userid。如果智能机器人创建者为企业超级管理员，则此字段为明文userid，否则此字段为企业主体下的加密userid（可通过 [自建应用与智能机器人的对接](#61328) 转为明文userid） |
...
sequenceDiagram
    ...
    C->>B: @机器人发消息
    B->>A: aibot_msg_callback (req_id=xxx)
    Note over A: 生成唯一的 stream.id

    A->>B: aibot_respond_msg (req_id=xxx, stream.id=abc, finish=false, content="正在查询...")
    Note over B: 首次使用该 stream.id<br/>创建新的流式消息
    B->>C: 展示流式消息

    A->>B: aibot_respond_msg (req_id=xxx, stream.id=abc, finish=false, content="广州今日天气：29度...")
    Note over B: 继续使用相同 stream.id<br/>刷新消息内容
    B->>C: 更新流式消息

    A->>B: aibot_respond_msg (req_id=xxx, stream.id=abc, finish=false, content="广州今日天气：29度，多云，降雨概率60%...")
    B->>C: 更新流式消息

    A->>B: aibot_respond_msg (req_id=xxx, stream.id=abc, finish=true, content="广州今日天气：29度，多云，降雨概率60%，建议携带雨具。")
    Note over B: finish=true<br/>流式消息结束
    B->>C: 展示最终消息

    rect rgb(255, 255, 224)
        Note over A, B: 10分钟超时限制：从首次发送开始计时
    end
```
